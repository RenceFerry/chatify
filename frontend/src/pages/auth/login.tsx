...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if(!validatedData.success) {
      setError({zodError: z.treeifyError(validatedData.error)});
      return;
    }
        ...
        <div className='w-[60%] shadow-md shadow-blueC min-w-[300px] max-w-[600px] rounded-xl py-20 flex flex-row items-center justify-center gap-3 bg-back px-5 md:px-10'>
          <div className="flex-1 flex flex-col jusitfy-center">
            <form onSubmit={handleSubmit} className='w-full flex flex-col justify-center items-start'>
              ...
              <div className='flex flex-col gap-2 mb-5 w-full'>
                <button type='submit' title='login' className='w-full bg-blueC h-14 rounded-xl grid items-center text-lg font-semibold text-blueA'>Log in</button>
